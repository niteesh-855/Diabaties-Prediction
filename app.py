...
def predict():
    if request.method == 'POST':
        ...
        sk = request.form.get("skin_thickness")
        ins = request.form.get("insulin")
        bmi = request.form.get("bmi")
        dpf = request.form.get("diabetes_pedigree_function")
        age = request.form.get("age")
        print(preg,glu,bp,sk,ins,bmi,dpf,age)

        with open("model.pickel","rb") as model_file:
            model = pickle.load(model_file)

        res = model.predict([[float(preg),float(glu),float(bp),float(sk),float(ins),float(bmi),float(dpf),float(age)]])
